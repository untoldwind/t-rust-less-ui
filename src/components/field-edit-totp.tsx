...
const FieldEditTOTPPopover: React.FC<{
  value: string;
  onChange: (newValue: string) => void;
}> = React.memo(({ value, onChange }) => {
  const otpUrl = new URL(
    value.startsWith("otpauth://") ? value : "otpauth://totp",
  );
  const otpParams = new URLSearchParams(otpUrl.search);
  const name = otpUrl.pathname.startsWith("/")
    ? otpUrl.pathname.substring(1)
    : otpUrl.pathname;
  ...
  return (
    <Grid columnSpec="min-content 1fr" gap={5} padding={5} alignItems="center">
      ...
      <InputGroup
        value={otpParams.get("issuer") || ""}
        fill
        onChange={updateIssuer}
      />
      <NoWrap>Secret</NoWrap>
      <InputGroup
        value={otpParams.get("secret") || ""}
        fill
...
export const FieldEditTOTP: React.FC<FieldEditTOTPProps> = React.memo(
          ...
          rightElement={
            <Flex flexDirection="row" gap={5} padding={[0, 5, 0, 0]}>
              <Popover
                ...
                content={
                  <FieldEditTOTPPopover value={value} onChange={onChange} />
                }
              >
