...
export function upsertStoreConfig(
...
export async function getSecretVersion(
  ...
  const secretVersion: Omit<SecretVersion, "id"> = await invoke(
    "store_get_version",
    {
      storeName,
      blockId,
...
export function addSecretVersion(
  storeName: string,
...
export function secretToClipboard(
  storeName: string,
  blockId: string,
  properties: string[],
): Promise<void> {
...
export async function selectStoreLocation(
  defaultPath?: string,
): Promise<string | null> {
